...
class Graph():
    ...
    def getMinumumKey(self, weight, visited):
        min = 9999
    ...
    def primsAlgo(self):
        ...
        for _ in range(self.vertices):
            minIndex = self.getMinumumKey(weight, visited)
            visited[minIndex] = True
            for vertex in range(self.vertices):
                if self.graph[minIndex][vertex] > 0 and visited[vertex] == False and weight[vertex] > self.graph[minIndex][vertex]:
